        ...
        switch (ctrlToArrow(event.keyDown.keyCode))
            ...
            default:
                for( int i = 0; i < strings->getCount(); i++ )
                    ...
                    char cpchar;
                    if( hab != NULLHANDLE )
	                    cpchar = WinUpperChar( hab, Codepage, Country, event.keyDown.charScan.charCode );
	                 else
	                 	  cpchar = toupper(event.keyDown.charScan.charCode);
                    if( getAltCode(c) == event.keyDown.keyCode || ( ( owner->phase == phPostProcess || (state & sfFocused) != 0 ) && c != 0 && cpchar == c ) )
                        {
								if (buttonState(i))
									{
									if ( focus())
