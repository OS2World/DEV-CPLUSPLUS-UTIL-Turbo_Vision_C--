...
ushort getAltCode(char c)
{
    ...
    if( hab == NULLHANDLE )
	    c = toupper(c);
	 	 ...
	 	 c = WinUpperChar( hab, Codepage, Country, c );

    /* SS: this makes g++ happy */

//    if( (unsigned char) c == '\xF0' )
    if( (unsigned char) c == (unsigned char) '\xF0' )
        return 0x200;       // special case to handle alt-Space

//    for( int i = 0; i < sizeof( altCodes1 ); i++)
    int i;
//    for( i = 0; i < sizeof( altCodes1 ); i++) /* XXX */
    for( i = 0; i < (int)sizeof( altCodes1 ); i++) /* XXX */
       if( altCodes1[i] == c )
        return (i+0x10) << 8;

//    for( i = 0; i < sizeof( altCodes2); i++) /* XXX */
    for( i = 0; i < (int)sizeof( altCodes2); i++) /* XXX */
        if (altCodes2[i] == c)
            return (i+0x78) << 8;
...
char TListViewer::separatorChar = 179;
