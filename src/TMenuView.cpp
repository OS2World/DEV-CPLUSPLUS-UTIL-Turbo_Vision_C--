        ...
        switch (e.what)
            ...
            case  evKeyDown:
                switch( ctrlToArrow(e.keyDown.keyCode) )
                    ...
                    case  kbUp:
                    case  kbDown:
//	messageBox( "1!!!", mfError | mfOKButton );
                        if( size.y != 1 )
...
{
//Orlik
char *Latin    ="qwertyuiop[]asdfghjkl;'zxcvbnm,.";
char *RussianLC="©æãª¥­£èé§åêäë¢ ¯à®«¤¦íïçá¬¨âì¡î";
char *RussianUC="‰–“Š…ƒ˜™‡•š”›‚€Ž‹„†Ÿ—‘Œˆ’œž";
char *pos, ch2;

//Orlik
	pos = strchr( RussianLC, ch );
	if( pos ) ch = Latin[ pos - RussianLC ];
	pos = strchr( RussianUC, ch );
	if( pos ) ch = Latin[ pos - RussianUC ];	//Orlik
   ch = toupper(ch);
    ...
    while( p != 0 )
        ...
        if( p->name != 0 && !p->disabled )
            ...
            if( loc != 0 )
            	{
            	ch2 = loc[1];	//Orlik
					pos = strchr( RussianLC, ch2 );
					if( pos ) ch2 = Latin[ pos - RussianLC ];
					pos = strchr( RussianUC, ch2 );
					if( pos ) ch2 = Latin[ pos - RussianUC ];	//Orlik
            	if ((uchar)ch == toupper( ch2 ) )
//            	if ((uchar)ch == toupper( loc[1] ) )
	                return p;
               }
            }
        p =  p->next;
