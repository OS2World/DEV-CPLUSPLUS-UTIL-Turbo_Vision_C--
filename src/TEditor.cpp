...
//int TEditor::charPos( ushort p, ushort target )	/* XXX */
uint TEditor::charPos( uint p, uint target )		/* XXX */
{
//    int pos = 0;	/* XXX */
    uint pos = 0;	/* XXX */
    while( p < target )
...
//ushort TEditor::charPtr( ushort p, int target )	/* XXX */
uint TEditor::charPtr( uint p, uint target )		/* XXX */
{
//  int pos = 0;	/* XXX */
    uint pos = 0;	/* XXX */
  /* SS: changed */
...
//void TEditor::deleteRange( ushort startPtr,	/* XXX */
//                           ushort endPtr,	/* XXX */
void TEditor::deleteRange( uint startPtr,	/* XXX */
                           uint endPtr,		/* XXX */
                           Boolean delSelect
                         )
{
...
//static unsigned char KoiTable[128] =	/* XXX */
static unsigned KoiTable[128] =		/* XXX */
{
...
//void TEditor::drawLines( int y, int count, ushort linePtr )	/* XXX */
void TEditor::drawLines( int y, uint count, uint linePtr )	/* XXX */
{
...
//ushort TEditor::getMousePtr( TPoint m )	/* XXX */
uint TEditor::getMousePtr( TPoint m )		/* XXX */
{
...
{
    selecting = False;
    uint selLen = selEnd - selStart;		/* XXX */
...
//    long newSize = long(bufLen + delCount - selLen + delLen) + length; /* XXX */
    uint newSize = uint(bufLen + delCount - selLen + delLen) + length; /* XXX */

//    if( newSize > bufLen + delCount )	/* XXX */
//        if( newSize > 0xFFE0l || setBufSize(ushort(newSize)) == False ) /* XXX */
    if( newSize > bufLen + delCount && setBufSize(newSize) == False ) /* XXX */
            {
            editorDialog( edOutOfMemory );
	    selEnd = selStart;
            return False;
...
//    ushort selLines = countLines( &buffer[bufPtr(selStart)], selLen ); /* XXX */
    uint selLines = countLines( &buffer[bufPtr(selStart)], selLen ); /* XXX */
    if( curPtr == selEnd )
        {
        if( allowUndo == True )
            {
...
//    ushort lines = countLines( &buffer[curPtr], length ); /* XXX */
    uint lines = countLines( &buffer[curPtr], length ); /* XXX */
    curPtr += length;
    curPos.y += lines;
    drawLine = curPos.y;
...
{
    uint pt = editor->bufPtr(editor->selStart);	/* XXX */
    return insertBuffer( editor->buffer,
//                         editor->bufPtr(editor->selStart),	/* XXX */
			 pt,					/* XXX */
...
//Boolean TEditor::insertText( const void *text, ushort length, Boolean selectText ) /* XXX */
Boolean TEditor::insertText( const void *text, uint length, Boolean selectText ) /* XXX */
{
...
//ushort TEditor::lineMove( ushort p, int count )	/* XXX */
uint TEditor::lineMove( uint p, int count )	/* XXX */
{
//    ushort i = p;	/* XXX */
    uint i = p;		/* XXX */
    p = lineStart(p);
//    int pos = charPos(p, i);	/* XXX */
    uint pos = charPos(p, i);	/* XXX */
    while( count != 0 )
        {
        i = p;
        if( count < 0 )
...
//    ushort p = lineStart(curPtr);	/* XXX */
//    ushort i = p;			/* XXX */
    uint p = lineStart(curPtr);	/* XXX */
    uint i = p;			/* XXX */
    while( i < curPtr &&
...
//ushort TEditor::nextLine( ushort p )	/* XXX */
uint TEditor::nextLine( uint p )	/* XXX */
{
    return nextChar(lineEnd(p));
}

//ushort TEditor::nextWord( ushort p )	/* XXX */
uint TEditor::nextWord( uint p )	/* XXX */
{
...
//ushort TEditor::prevLine( ushort p )	/* XXX */
uint TEditor::prevLine( uint p )	/* XXX */
{
  return lineStart(prevChar(p));
}

//ushort TEditor::prevWord( ushort p )	/* XXX */
uint TEditor::prevWord( uint p )	/* XXX */
{
...
{
//    ushort pos = curPtr;	/* XXX */
//    ushort i;			/* XXX */ 
    ulong pos = curPtr;	/* XXX */
    ulong i;		/* XXX */ 
    do  {
...
//void TEditor::setBufLen( ushort length )	/* XXX */
void TEditor::setBufLen( uint length )	/* XXX */
{
    bufLen = length;
...
//Boolean TEditor::setBufSize( ushort newSize )	/* XXX */
Boolean TEditor::setBufSize( uint newSize )	/* XXX */
{
...
//void TEditor::setCurPtr( ushort p, uchar selectMode )	/* XXX */
void TEditor::setCurPtr( uint p, uchar selectMode )	/* XXX */
{
//    ushort anchor;	/* XXX */
    uint anchor;	/* XXX */
...
//void TEditor::setSelect( ushort newStart, ushort newEnd, Boolean curStart ) /* XXX */
void TEditor::setSelect( uint newStart, uint newEnd, Boolean curStart ) /* XXX */
{
//    ushort p;	/* XXX */
    uint p;	/* XXX */
    if( curStart != 0 )
...
void TEditor::setWidth( int Size )
{
	widthSize = Size;
}
