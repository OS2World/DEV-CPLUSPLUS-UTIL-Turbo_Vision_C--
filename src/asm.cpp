...
size_t iScan( const void *block, size_t size, const char *str )
{
  ...
  while(i--) x =  (x<<5)%q;
	...
	if(hab != NULLHANDLE)
	{
		for (i=0; i<patternLength; i++) {
			ch = *patP++;
			ch = WinUpperChar( hab, Codepage, Country, ch );
			patternHash = ( (patternHash<<5) + ch ) % q;
			ch = *testP++;
			ch = WinUpperChar( hab, Codepage, Country, ch );
			testHash    = ( (testHash   <<5) + ch ) % q;
		}
	}
	else
	{
  ...
  while (1) {
     ...
     // Advance & calculate the new hash value:
		if(hab != NULLHANDLE)
		{
			ch = *testP;
			ch = WinUpperChar( hab, Codepage, Country, ch );
			testHash = ( testHash + q32 - ch * x ) % q;
			ch = *(patternLength + testP++);
			ch = WinUpperChar( hab, Codepage, Country, ch );
			testHash = ( (testHash<<5)  + ch ) % q;
		}
		else
...
//char TEditor::bufChar( ushort p ) /* XXX */
char TEditor::bufChar( uint p )	/* XXX */
{
    return buffer[p + ((p >= curPtr) ? gapLen : 0)];
}

//ushort TEditor::bufPtr( ushort p ) /* XXX */
uint TEditor::bufPtr( uint p ) /* XXX */
{
    return (p >= curPtr) ? p + gapLen : p;
}

#if 0
//void TEditor::formatLine( void *DrawBuf, ushort LinePtr, /* XXX */
void TEditor::formatLine( void *DrawBuf, uint LinePtr, /* XXX */
                          int Width, ushort Color )
{
//    ushort p = LinePtr; /* XXX */
    uint p = LinePtr; /* XXX */
                          ...
                          int Width, ushort Color )
{
  ...
  while ((p < curPtr) && (buffer[p] != '\n') && (i <= Width)) {
    ...
    if (buffer[p] == 0x9) {
      ...
      ((ushort *) DrawBuf) [i] = curColor | (uchar)buffer[p]; /* XXX */
      p++; i++;
    }
  }
...
//ushort TEditor::lineEnd( ushort p ) /* XXX */
uint TEditor::lineEnd( uint p ) /* XXX */
{
/*
    while (p < curPtr)
        if (buffer[p] == 0x0D)
...
//ushort TEditor::lineStart( ushort p ) /* XXX */
uint TEditor::lineStart( uint p ) /* XXX */
{
/*
...
//ushort TEditor::nextChar( ushort p ) /* XXX */
uint TEditor::nextChar( uint p ) /* XXX */
{
    if (p == bufLen)   return p;
    if (++p == bufLen) return p;

    /* SS: changed */
//    int t = (p >= curPtr) ? p + gapLen : p; /* XXX */
//    ulong t = (p >= curPtr) ? p + gapLen : p;

    return p;
//    return ((buffer [t-1] == 0x0D) && (buffer [t] == 0x0A)) ? p + 1 : p;
}

//ushort TEditor::prevChar( ushort p ) /* XXX */
uint TEditor::prevChar( uint p ) /* XXX */
{
    if (p == 0)   return p;
    if (--p == 0) return p;
    ...
    /* SS: changed */
//    int t = (p >= curPtr) ? p + gapLen : p; /* XXX */
//    uint t = (p >= curPtr) ? p + gapLen : p; /* XXX */
...
//  unsigned char* src=frameMask; /* XXX */
