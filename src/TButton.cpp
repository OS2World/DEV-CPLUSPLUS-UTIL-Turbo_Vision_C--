...
void TButton::handleEvent( TEvent& event )
{
    ...
    switch( event.what )
        ...
        case evKeyDown:
        		if( hab == NULLHANDLE )
        		{
            if( event.keyDown.keyCode == getAltCode(c) || ( owner->phase == phPostProcess && c != 0 && toupper(event.keyDown.charScan.charCode) == c ) || ( (state & sfFocused) != 0 && event.keyDown.charScan.charCode == ' ' ) )
                {  press(); clearEvent( event ); }
            }
            else
            {
            if( event.keyDown.keyCode == getAltCode(c) || ( owner->phase == phPostProcess && c != 0 && WinUpperChar( hab, Codepage, Country, event.keyDown.charScan.charCode ) == c ) || ( (state & sfFocused) != 0 && event.keyDown.charScan.charCode == ' ' ) )
                {  press(); clearEvent( event ); }
            }
            break;
